...
<html>
<head>
  <title>Pesan Pembeli | HanTok</title>
...
include("koneksi.php");
?>
<div class="container"><center><div class="alert alert-dark" role="alert"><h1>PESAN PEMBELI HANTOK</h1></center>
<?php if (!empty($_GET['update'])){
if ($_GET['update']==1)
echo "<p class='message'> <h4>Data Pembeli berhasil diupdate</h4></p>";

else if ($_GET['update']==2)
echo "<p class='message'> <h4>Data Pembeli berhasil ditambahkan</h4></p>";

else if ($_GET['update']==3)
echo "<p class='message'> <h4>Data Pembeli berhasil dihapus</h4></p>";
}
?>
<?php
$no=1;
$sqlCount = "select count(nama) from pesan";
...
$sql_limit = "select * from pesan order by nama limit
